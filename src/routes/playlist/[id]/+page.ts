...
export const load: PageLoad = async ({ fetch: _fetch, params, depends, route, url, parent }) => {
	...
	const [playlistResponse, isFollowingRes] = await Promise.all([
	...
	const playlistResJSON: SpotifyApi.SinglePlaylistResponse = await playlistResponse.json();
	...
	if (page && page !== '1') {
		...
		const trackResJSON = await tracksRes.json();
